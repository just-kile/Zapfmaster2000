...
echo ' <div class="news-backgrnd"><div class="newsfeed">
  ...
  $ITEMS_PER_PAGE  = 8;
	  ...
	  $downBorder =  $page *$ITEMS_PER_PAGE ;
	
	  
	  $ergebnis =  mysql_query("SELECT * FROM t_news ORDER BY NEWS_ID DESC LIMIT ".$downBorder.",".  $ITEMS_PER_PAGE ) or die("Fehler:".mysql_error($conn)."<br><br>$sql");
	  ...
	  while($row = mysql_fetch_object($ergebnis))
	  {
		  ...
		  if($type==="DRAWING"){
			 ...
			 // $entry = getGainedAchievementById($ACH_ID);
			  ...
			  $ergebns = mysql_query("SELECT GAINED_ACHIEVEMENT_ID,ACHIEVEMENT_ID,USER_ID,DATE FROM T_GAINED_ACHIEVEMENTS WHERE GROUP_ID='".$ACH_ID."'") or die("Fehler:".mysql_error($conn)."<br><br>$sql");
...
$erg =  mysql_query("SELECT count(*) as summe FROM t_news") or die("Fehlr:".mysql_error($conn)."<br><br>$sql");
 ...
 if($entry = mysql_fetch_object($erg)){
		...
		$complete = $entry->summe;	
		 $pages = ceil($complete / $ITEMS_PER_PAGE);
		//echo $pages;
